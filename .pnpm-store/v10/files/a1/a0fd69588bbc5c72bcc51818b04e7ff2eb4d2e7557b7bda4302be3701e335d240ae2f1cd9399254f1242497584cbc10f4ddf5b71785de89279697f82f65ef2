...
async function getMulticallSetClaimConditionTransactions(options) {
    ...
    const phases = await Promise.all(options.phases.map(async (phase) => {
        ...
        return {
            ...
            maxClaimableSupply: phase.maxClaimableSupply ?? Solidity_1.maxUint256,
            merkleRoot,
            metadata,
            ...
            quantityLimitPerWallet: phase.maxClaimablePerWallet ?? Solidity_1.maxUint256,
    ...
    if (Object.keys(merkleInfos).length > 0) {
        ...
        for (const key of Object.keys(metadata.merkle || {})) {
            const merkleInfo = (0, type_guards_js_1.isRecord)(metadata.merkle)
                ? metadata.merkle[key]
                : undefined;
            if (merkleInfo) {
    ...
    if (options.tokenId !== undefined) {
        ...
        if (options.singlePhase) {
            const { encodeSetClaimConditions } = await Promise.resolve().then(() => require("../../../extensions/erc1155/__generated__/IDropSinglePhase1155/write/setClaimConditions.js"));
        ...
        // erc721 or erc20
        if (options.singlePhase) {
            const { encodeSetClaimConditions } = await Promise.resolve().then(() => require("../../../extensions/erc721/__generated__/IDropSinglePhase/write/setClaimConditions.js"));
            const phase = sortedPhases[0];
            if (!phase) {
                throw new Error("No phase provided");
            }
