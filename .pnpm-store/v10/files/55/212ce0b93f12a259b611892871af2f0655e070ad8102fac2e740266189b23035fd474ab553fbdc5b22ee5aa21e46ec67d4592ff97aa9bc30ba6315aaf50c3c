...
export function useStepExecutor(options) {
    ...
    // Execute batch transactions
    const executeSendCalls = useCallback(async (txs, wallet, account, completedStatusResults, abortSignal) => {
        if (typeof preparedQuote?.type === "undefined") {
            throw new Error("No quote generated. This is unexpected.");
        }
        ...
        const { prepareTransaction } = await import("../../../transaction/prepare-transaction.js");
        const { sendCalls } = await import("../../../wallets/eip5792/send-calls.js");
        if (txs.length === 0) {
            throw new Error("No transactions to batch");
        }
        const firstTx = txs[0];
