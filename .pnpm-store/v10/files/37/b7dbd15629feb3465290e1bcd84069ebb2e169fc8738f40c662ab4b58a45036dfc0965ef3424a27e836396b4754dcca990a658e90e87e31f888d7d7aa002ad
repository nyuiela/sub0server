...
function saveSnapshot(options) {
    return (0, setContractURI_js_1.setContractURI)({
        asyncParams: async () => {
            const merkleInfos = {};
            // need to upload merkle tree info to the contract metadata
            const [{ getContractMetadata }, { contractURI }] = await Promise.all([
                Promise.resolve().then(() => require("../../common/read/getContractMetadata.js")),
                Promise.resolve().then(() => require("../../common/__generated__/IContractMetadata/read/contractURI.js")),
            ]);
