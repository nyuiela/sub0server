...
const from_eip1193_js_1 = require("./eip1193/from-eip1193.js");
const to_eip1193_js_1 = require("./eip1193/to-eip1193.js");
/**
...
async function toViemContract(options) {
    return {
        ...
        address: (0, address_js_1.getAddress)(options.thirdwebContract.address),
    };
}
    ...
    const transport = (0, viem_1.custom)({
        request: async (request) => {
            ...
            if (request.method === "wallet_switchEthereumChain" ||
                ...
                throw new Error("Can't switch chains because only an account was passed to 'viemAdapter.walletClient.toViem()', please pass a connected wallet instance instead.");
            }
            return rpcClient(request);
        },
    });
    return (0, viem_1.createWalletClient)({
        account: (0, address_js_1.getAddress)(account.address),
        chain: viemChain,
        key: "thirdweb-wallet",
        transport,
    });
...
function walletToViem(options) {
    ...
    const eip1193Provider = (0, to_eip1193_js_1.toProvider)({
        chain,
        client,
        wallet,
    });
...
function walletFromViem(options) {
    ...
    const wallet = (0, from_eip1193_js_1.fromProvider)({
        provider: {
            on: () => { },
            removeListener: () => { },
