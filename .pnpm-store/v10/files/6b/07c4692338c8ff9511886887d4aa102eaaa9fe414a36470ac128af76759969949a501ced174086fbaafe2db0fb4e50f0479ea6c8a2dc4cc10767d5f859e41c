...
exports.fetchProofsERC20 = fetchProofsERC20;
...
const convert_quantity_js_1 = require("../drops/convert-quantity.js");
const hash_entry_erc20_js_1 = require("./hash-entry-erc20.js");
/**
 ...
 * @extension AIRDROP
 */
async function fetchProofsERC20(options) {
    ...
    const merkleData = (0, type_guards_js_1.isRecord)(metadata.merkle) ? metadata.merkle : {};
    ...
    const hashedEntries = await Promise.all(shardData.entries.map(async (entry) => {
        return (0, hash_entry_erc20_js_1.hashEntryERC20)({
            entry,
    ...
    const proof = tree
        .getHexProof(await (0, hash_entry_erc20_js_1.hashEntryERC20)({
        entry,
        tokenDecimals: options.tokenDecimals,
    }))
        .concat(shardData.proofs);
    const quantity = (0, convert_quantity_js_1.convertQuantity)({
...
//# sourceMappingURL=fetch-proofs-erc20.js.map