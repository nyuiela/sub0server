...
export function useSendTransaction(config = {}) {
    ...
    const showPayModal = (data) => {
        if (payModal === false)
            return;
        setRootEl(_jsx(TransactionModal, { country: payModal?.country, currency: payModal?.currency, client: data.tx.client, localeId: payModal?.locale || "en_US", modalMode: data.mode, onClose: () => {
                setRootEl(null);
