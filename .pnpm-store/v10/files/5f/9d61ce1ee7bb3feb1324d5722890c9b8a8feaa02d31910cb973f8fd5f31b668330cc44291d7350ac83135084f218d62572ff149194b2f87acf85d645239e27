...
export async function getMulticallSetClaimConditionTransactions(options) {
    ...
    if (options.tokenId !== undefined) {
        ...
        if (options.singlePhase) {
            const { encodeSetClaimConditions } = await import("../../../extensions/erc1155/__generated__/IDropSinglePhase1155/write/setClaimConditions.js");
        ...
        // erc721 or erc20
        if (options.singlePhase) {
            const { encodeSetClaimConditions } = await import("../../../extensions/erc721/__generated__/IDropSinglePhase/write/setClaimConditions.js");
            const phase = sortedPhases[0];
            if (!phase) {
                throw new Error("No phase provided");
            }
