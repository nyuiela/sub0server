...
export function TransactionModal(props) {
    ...
    return (_jsx(CustomThemeProvider, { theme: props.theme, children: _jsx(Modal, { className: "tw-modal__view-transaction", title: "View Transaction", open: true, setOpen: (_open) => {
                if (!_open) {
                    props.onClose();
                }
...
function TransactionModalContent(props) {
    if (props.modalMode === "deposit") {
        return _jsx(DepositAndExecuteTx, { ...props });
    }
    return (_jsx(TransactionWidgetContentWrapper, { client: props.client, country: props.country, currency: props.currency, transaction: props.tx, onCancel: props.onClose, locale: props.localeId, onSuccess: (data) => {
            props.onTxSent(data);
...
function DepositAndExecuteTx(props) {
    ...
    if (screen === "execute-tx") {
        return (_jsx(ExecutingTxScreen, { onBack: () => {
                setScreen("deposit");
            }, closeModal: props.onClose, onTxSent: props.onTxSent, tx: props.tx, windowAdapter: webWindowAdapter }));
    }
    if (screen === "deposit") {
        return (_jsx(DepositScreen, { client: props.client, connectLocale: localeQuery.data, onDone: () => {
