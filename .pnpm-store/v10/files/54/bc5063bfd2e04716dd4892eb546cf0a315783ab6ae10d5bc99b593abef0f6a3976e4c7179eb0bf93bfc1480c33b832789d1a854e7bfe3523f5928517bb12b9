...
exports.getZkGasFees = getZkGasFees;
...
async function populateEip712Transaction(options) {
    const { account, transaction } = options;
    const { gas, maxFeePerGas, maxPriorityFeePerGas, gasPerPubdata } = await getZkGasFees({ from: (0, address_js_1.getAddress)(account.address), transaction });
    ...
    if (gas === undefined ||
        ...
        try {
            const result = (await rpc({
                ...
                // biome-ignore lint/suspicious/noExplicitAny: TODO add to RPC method types
                params: [(0, bigint_js_1.replaceBigInts)(params, hex_js_1.toHex)],
            }));
            ...
            const [{ estimateGas }, { getDefaultGasOverrides }] = await Promise.all([
                Promise.resolve().then(() => require("../estimate-gas.js")),
                Promise.resolve().then(() => require("../../../gas/fee-data.js")),
            ]);
            ...
            if ("maxPriorityFeePerGas" in gasOverrides &&
